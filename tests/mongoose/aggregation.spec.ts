...
test.describe('MongoDB Advanced Aggregation Operations with Mongoose', () => {
  test('should perform $lookup to join collections', async ({ DocumentModel, UserModel, FolderModel }) => {
  ...
  test('should use $facet for multiple aggregation pipelines', async ({ DocumentModel, UserModel, FolderModel }) => {
    // ARRANGE: Create test data
    const userData = TestDataFactory.createUser();
    const user = await UserModel.create(userData);
    const testUserId = user._id;
  ...
  test('should use $unwind for array operations', async ({ DocumentModel, UserModel, FolderModel }) => {
    // ARRANGE: Create test data
    const userData = TestDataFactory.createUser();
    const user = await UserModel.create(userData);
    const testUserId = user._id;
  ...
  test('should use $addFields for computed fields', async ({ DocumentModel, UserModel, FolderModel }) => {
    // ARRANGE: Create test data
    const userData = TestDataFactory.createUser();
    const user = await UserModel.create(userData);
    const testUserId = user._id;
